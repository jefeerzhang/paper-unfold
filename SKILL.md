...
> 本节是**铁律 3.5** 的具体执行规范。详见 [`reference/sciverse.md`](reference/sciverse.md)——含 token 检测时机、step-by-step 配置指南、用户决策表与禁止事项。

Agent 调用 `mcp__sciverse__*` 遇到鉴权失败、或用户首次使用本 Skill 时，按 `reference/sciverse.md` 引导用户完成配置（**严禁静默降级**）。
...
工具优先级表、关键原则与 SciVerse MCP 工具速查详见 [`reference/sciverse.md`](reference/sciverse.md)。

速记：PDF 提取走 `pdftotext` → `pymupdf` → `pdfplumber` → 手动粘贴；学术检索 SciVerse 优先，失败须显式 fallback。
