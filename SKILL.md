...
description: |
  ...
  支持「识别研究空白」专项模式：基于 Miles 七分法逐项分析 + SciVerse 交叉验证。
...
> 本节与上方「文献导读」为**并列的第二工作流**。用户请求「识别研究空白」「分析 gap」「找研究空白」时进入此模式。

### 触发条件

以下任一条件满足即进入本模式：
- 用户上传 PDF 并明确要求「识别研究空白 / 分析 gap / 找研究空白」
- 用户在完成一次文献导读后追加请求：「基于这篇论文，识别研究空白」
- 用户粘贴论文文本并要求空白分析
...
与文献导读共享「阶段 1：PDF 提取」逻辑（铁律 0 + 铁律 2）。

额外支持：
- 用户直接粘贴论文文本（跳过 PDF 提取）
- 用户提供已完成的导读报告（从报告中提取结构化信息）
...
**强制要求：** 报告顶部必须包含局限性声明（见输出结构中的 ⚠️ 提示），不得省略。此声明提醒用户：分析基于单篇论文 + 有限检索，不等于穷尽性文献综述。
...
（从标记为「仍开放」的空白中，推荐 1-3 个最值得跟进的方向）

### 方向 1：<空白标题>
- **为什么值得填补**：（对学科/实践的意义）
- **建议研究角度**：（方法/人群/理论层面的具体建议）
- **推荐阅读**：（SciVerse 检索的相关文献，含 doc_id）
...
**识别研究空白模式：**
- [ ] 报告顶部包含 ⚠️ 局限性声明（基于单篇论文 + 有限检索，不保证穷尽）
- [ ] 七项空白扫描全部完成（不允许跳过任何一项）
- [ ] 每项空白包含三要素：存在性判断 + 证据 + 置信度
- [ ] 空白地图表格完整（7 行）
- [ ] 标记为「✅ 存在」的空白均经过 SciVerse 交叉验证
- [ ] 高价值空白建议 ≤3 个（避免信息过载）
- [ ] 每个建议方向附推荐阅读（含 doc_id）
- [ ] 报告已保存到 `./研究空白/`
...
| 用户在导读完成后追加「识别空白」 | 复用阶段 G2-G5，跳过 G1（PDF 已提取）；在报告中注明「基于前述导读结果」 |
